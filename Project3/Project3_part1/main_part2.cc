...
os << "   logic [" << ceil(log2(N)) - 1 << ":0] en_out, Addr_X;" << endl;
...
os << "" << endl;
os << "" << endl;
os << "   controlpath ctrl(clk, reset, s_valid, m_ready, m_valid, s_ready, en_f, en_check, en_out, Wr_en_X, accum_src, Addr_X," << endl;
	for (int i=0; i < P;i++)
	    if (i == P - 1)
	...
	for (int i = 0; i < P; i++ )
	    os << "   datapath data" << i + 1 <<"(clk, reset, data_in, Addr_X, Addr_W" << i + 1 <<", Addr_B" << i + 1 <<", Wr_en_X, data_out" << i + 1 <<", en_f, en_check, accum_src);" << endl;	    

os << "" << endl;
os << "endmodule" << endl;
os << "" << endl;

os << "module controlpath (clk, reset, s_valid, m_ready, m_valid, s_ready, en_f, en_check, en_out, Wr_en_X, accum_src, Addr_X," << endl;
	for (int i=0; i < P;i++)
	    if (i == P - 1)
		os << "			Addr_W"<< P <<", Addr_B" << P << ");" << endl;
	    else
...
os << "	  output logic [" << ceil(log2(N)) - 1 << ":0] en_out, Addr_X;	" << endl;
